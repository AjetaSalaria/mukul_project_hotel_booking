...
<div id="restaurant-section">
 ...
 -->
    ...
    <div class="container-fluid">
        ...
        <div class="restaurant_content-sec card room-box-sec room-box-new">
        ...
        @foreach($rcategories as $categoryName=>$categoryItems)
        @if($categoryItems[0]->category->status == 'active')

        <div class="category-item-wrapper catg-list room_menu" id="category-item-wrapper-{{$categoryItems[0]->category_id}}">
            <div class="row page-titles mx-0">
                                    ...
                                    <div class="food-items-sec">
                                        ...
                                        <div class="price-box d-flex justify-content-end align-items-center">
                                            ...
                                            <div class="text-right">
                                                <button  id="order_item" data-value="{{json_encode($item->only(['name','id','price','details','discount','discount_type','discount_to']))}}" class="btn btn-xxs btn-info add-to-cart custom-lnk-{{$item->id}}" data-link={{$item->id}} >{{trans('layout.add')}}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
     ...
     <div class="add-overview col-sm-11" id="add-overview">
        <form action="{{route('order.place')}}" method="post" id="orderForm">
            ...
            <input type="hidden" name="category_type" value="room" id="service_custom_type">
                            ...
                            <div class="order-items-payment">
                                <div class="row mid-payment">
                                    <div class="col-md-12">
                                        ...
                                        <input value="{{auth()->check()?auth()->user()->name:''}}" name="name" type="text" class="form-control" id="name" placeholder="Ex: Jone Doe" required>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <input name="whatsappnum" type="number" class="form-control" id="whatsappnum" placeholder="Ex: 9988898888" required>
                                      </div>
                                    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
       //$(custom-lnk-{
        ...
        var link = '<?php if(!empty($_GET['link'])) {
            echo $_GET['link'];
        }else{
            echo '';
        } ;?>';
        ...
        var custom_link_tab = 'custom-lnk-'+link;
        const item = JSON.parse($('.custom-lnk-'+link).attr('data-value'));
...
<script !src="">
    ...
    $(document).on('click', '.roomitem-category', function(e) {
        e.preventDefault();
    ...
    $(document).on('click', '#show_service', function(e) {
        e.preventDefault();
        $('body').removeClass("myordercontent");
        $('.hamburger').click();
        $('#restaurant-section').show();
    ...
    $(document).on('click', '.add-to-cart', function(e) {
        ...
        e.preventDefault();
        let service_custom_type =  $('#service_custom_type').val();
        $('#add-overview').animate({
            bottom: '0px'
        });
        $('#orderOverviewSection').show();
    ...
    $(document).on('click', '.minus-quantity', function(e) {
       ...
       let minus_qty = $(this).parent().find('.item-quantity').text();
    ...
    $(document).on('keyup click', '#txtSearchs,#txtSearch', function(e) {
        ...
        if(search.length == 0){
            ...
            currentRequest = $.ajax({
                type:"GET",
                url: '/restaurants/roomitems/search',
                beforeSend : function()    {
