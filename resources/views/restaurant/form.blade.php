...
<div>
    ...
    <section>
        <div class="row">
            ...
            <div class="col-lg-12 mb-2">
                <div class="form-group">
                    <label class="text-label">{{trans('layout.background_image')}}</label>
                    @if(isset($restaurant) && $restaurant->background_image)
                        <img style="max-width: 50px" src="{{asset('uploads').'/'.$restaurant->background_image}}" alt="{{$restaurant->background_image}}">
                    @endif
                    <input type="file" name="background_file" class="form-control" accept="image/*">
                </div>
            </div>
        <div class="col-lg-12 mb-2">
                <div class="form-group">
                    <label class="text-label">{{trans('layout.room_service_image')}}</label>
                    @if(isset($restaurant) && $restaurant->roomservice_image)
                        <img style="max-width: 50px" src="{{asset('uploads').'/'.$restaurant->roomservice_image}}" alt="{{$restaurant->roomservice_image}}">
                    @endif
                    <input type="file" name="room_service_file" class="form-control" accept="image/*">
                </div>
            </div>
            <div class="col-lg-12 mb-2">
                <div class="form-group">
                    <label class="text-label">{{trans('layout.food_service_image')}}</label>
                    @if(isset($restaurant) && $restaurant->foodservice_image)
                        <img style="max-width: 50px" src="{{asset('uploads').'/'.$restaurant->foodservice_image}}" alt="{{$restaurant->foodservice_image}}">
                    @endif
                    <input type="file" name="food_service_file" class="form-control" accept="image/*">
                </div>
            </div>
        </div>
    </section>
